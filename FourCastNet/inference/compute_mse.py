...
DECORRELATION_TIME = 24*9

idxes = {'T2M': 7, 'T500': 8, 'U500': 13}
fld = 'T2M'
...
def setup(params):
    ...
    if params.nettype == 'afno':
      model = AFNONet(params, img_size=img_size).to(device) 
    else:
    ...
    if params.log_to_screen:
        ...
        logging.info('Inference data from {} to {}'.format(files_paths[0], files_paths[-1]))

    return valid_dataset, model


def autoregressive_inference(params, ic, valid_data_full, model): 
    ic = int(ic + 1) if params['backward'] else int(ic)
    ...
    with torch.no_grad():
      for i in range(valid_data.shape[0]): 
        ...
        acc[i] = weighted_acc_torch_channels(pred, tar) #TODO use daily climatology like original?
        ...
        if params.interp > 0:
            ...
            valid_loss_coarse[i] = weighted_rmse_torch_channels(pred, tar)
    ...
    def get_day(hours):
        date = datetime(1991, 1, 1, 0, 0, 0) + timedelta(hours=hours)
        return "{:04d}{:02d}{:02d}".format(date.year, date.month, date.day)
    #np.save(os.path.join(params['experiment_dir'], "ground_truth{}.npy".format(get_day(ic))), seq_real)
    #np.save(os.path.join(params['experiment_dir'], "prediction{}.npy".format(get_day(ic))), seq_pred)
    mse = np.mean((seq_real - seq_pred) * (seq_real - seq_pred), axis=(-2,-1))
    np.save(os.path.join(params['experiment_dir'], "mse", "mse{}.npy".format(get_day(ic))), mse)
...
if __name__ == '__main__':
    ...
    n_samples_per_year = 9496 * 24 #365*24*3 #366*24
...
#        seq_pred = np.concatenate((seq_pred, sp), 0)
    ...
    sys.exit() # TODO: not saving for now
