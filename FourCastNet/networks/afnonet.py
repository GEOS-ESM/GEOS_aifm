...
class AFNO2D(nn.Module):
    def __init__(self, hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1, downsample_size=(24,48)):
        super().__init__()
        ...
        self.forward_transform = harmonics.RealSHT(H, W, grid="equiangular")
        self.inverse_transform = harmonics.InverseRealSHT(H, W, grid="equiangular")
    ...
    def forward(self, x):
        ...
        x = torch.movedim(self.forward_transform(torch.movedim(x, -1, -3)), -3, -1)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            x = torch.movedim(self.inverse_transform(torch.movedim(x, -1, -3)), -3, -1)
        x = x.type(dtype)
...
class Block(nn.Module):
    def __init__(
            ...
            downsample_size=(24,48),
        ):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.filter = AFNO2D(dim, num_blocks, sparsity_threshold, hard_thresholding_fraction, downsample_size=downsample_size) 
...
class AFNONet(nn.Module):
    def __init__(
        ...
        self.patch_size = (params.patch_size_y, params.patch_size_x)
        self.in_chans = params.N_in_channels
        self.out_chans = params.N_out_channels
...
if __name__ == "__main__":
    model = AFNONet(params, img_size=(40, 72), patch_size=(4,4), in_chans=6, out_chans=6)
    sample = torch.randn(1, 6, 40, 72)
    result = model(sample)
    print(result.shape)
